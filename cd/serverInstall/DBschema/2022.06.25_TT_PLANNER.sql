--------------------------------------------------------
--  DDL for Package TT_PLANNER
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE "PLANNER"."TT_PLANNER" is 
...
/
--------------------------------------------------------
--  DDL for Package Body TT_PLANNER
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "PLANNER"."TT_PLANNER" is
...
begin
    if (pClearMode='Y') then 
     delete from tt_cla where tt_comb_id in (select id from tt_combinations where weight = 122 and per_id is not null and integration_id is not null);
    ...
    for rec in (select unique per_id from tt_combinations where weight = 122 and per_id is not null and integration_id is not null) loop
