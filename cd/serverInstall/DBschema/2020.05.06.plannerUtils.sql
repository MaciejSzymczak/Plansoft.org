  ...
  begin
    ...
    begin      
      ...
      loop
        ...
        if instr(pcalc_lec_ids,rec.child_id)=0 and pno_conflict_flag is null then 
    ...
    for t in 1 .. xxmsz_tools.wordcount(pcalc_gro_ids, ';') loop
      ...
      loop
        if instr(pcalc_gro_ids,rec.child_id)=0  and pno_conflict_flag is null then 
    ...
    for t in 1 .. xxmsz_tools.wordcount(pcalc_rom_ids, ';') loop
      ...
      loop
        if instr(pcalc_rom_ids,rec.child_id)=0  and pno_conflict_flag is null then 
    ...
    if (pres_type='G') then
         ...
         for rec in (
           ...
           (
             select existing_dep.desc2, existing_dep.id
                  , (
                    select listagg(gro.abbreviation, '; ') within group (order by gro.abbreviation)
                      from gro_cla
                         , groups gro 
                    where no_conflict_flag is null
         ...
         for rec in (
            ...
            select unique day, hour  
              from gro_cla 
            where no_conflict_flag is null
             and gro_id in (select Id from helper3)
            and day between vdate_from and vdate_to
