create or replace PACKAGE TT_PLANNER is 
   ...
   | 2022.02    Two subjects   
   | 2022.01.10 changes: recalc_combination122   
   | 2022.11.30 changes: recalc_combination122 - pPER_ID
   | 2023.02.10 set_res_limitation122
   | 2023.12.04 round 4
   \-----------------------------------------------------------------------------------------------------------------------------*/ 
...
procedure set_res_limitation122 ( p_pla_id number, p_res_ids  varchar2, p_rescat_id number); 
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
procedure set_res_limitation122 ( p_pla_id number, p_res_ids  varchar2, p_rescat_id number) is
    l_res_ids     varchar2(32000) := replace(p_res_ids, ';', ',' );
    l_res_id     varchar2(100);
    l_elem_count number;
begin
  if disable_res_limitation then return; end if;
  wlog('set_res_limitation122(+)', 'p_res_ids=' || p_res_ids ||' p_rescat_id=' || p_rescat_id);
  delete from tt_ids;
  if p_res_ids is null then 
    -- no limitation
    return; 
  ...
  for t in 1 .. l_elem_count  loop
    l_res_id    := regexp_substr(l_res_ids, '([0-9]|[-])+', 1, t/*get t-th element*/); 
    insert into tmp_numbers (id) values(l_res_id);
  end loop;
  ...
  insert into tt_ids 
   select  
       case p_rescat_id 
        when g_lecturer then lec_id
        when g_subject  then sub_id
        when g_form     then for_id 
        when g_group    then gro_id
        when g_period   then per_id
        when g_planner  then pla_id
        else null  
      end case
    , id 
   from tt_combinations ttc 
    where --filter by lecturer. Ignore filter is there is no lecturer. Ignore lecturer is you are to select another lecturer
          (lec_id in (select Id from lecturers where id in (select Id from tmp_numbers)) OR (select count(Id) from lecturers where id in (select Id from tmp_numbers) and rownum<=1) =0 OR p_rescat_id=g_lecturer ) 
     and  (sub_id in (select Id from subjects  where id in (select Id from tmp_numbers)) OR (select count(Id) from subjects  where id in (select Id from tmp_numbers) and rownum<=1) =0 OR p_rescat_id=g_subject)
     and  (for_id in (select Id from forms     where id in (select Id from tmp_numbers)) OR (select count(Id) from forms     where id in (select Id from tmp_numbers) and rownum<=1) =0 OR p_rescat_id=g_form)
     and  (gro_id in (select Id from groups    where id in (select Id from tmp_numbers)) OR (select count(Id) from groups    where id in (select Id from tmp_numbers) and rownum<=1) =0 OR p_rescat_id=g_group)
     and  (per_id in (select Id from periods   where id in (select Id from tmp_numbers)) OR (select count(Id) from periods   where id in (select Id from tmp_numbers) and rownum<=1) =0 OR p_rescat_id=g_period)
     and ttc.enabled = 'Y' 
     and (avail_type='N' or (avail_type='L' and avail_curr > 0))
     --and rescat_comb_id in (select id from tt_rescat_combinations where combination_number = 122)
     and rescat_comb_id in (select rescat_comb_id from tt_pla where pla_id= p_pla_id);

  wlog('set_res_limitation122(-)', 'found:' || SQL%ROWCOUNT );    

  if (SQL%ROWCOUNT=0) then
    --0 records means "show all". dummy record means "show nothing"
    insert into tt_ids values (null, null);
  end if;
end;
...
begin
  ...
  for rec in (
    select id, rowid, weight
     ...
     --
     update tt_combinations 
        set  avail_curr = avail_orig - round(l_fill_sum,grid_decimal_places)
      where rowid = rec.rowid; 
  end loop;
...
begin
    ...
    for rec in (select unique per_id from tt_combinations where  id = nvl(p_tt_comb_id, id) and weight = 122 and per_id =nvl(pPER_ID, per_id) /*and integration_id is not null*/) loop
        ...
        merge into tt_combinations m 
            using (
                select lec_id, gro_id, nvl(str_elems.child_id, sub_id) sub_id, for_id, round(sum(fill/100*g.duration),grid_decimal_places) units --/count(unique cla.id)
                 from classes cla
                 , grids g
                 , gro_cla
                 , lec_cla
            ...
            using (
                select * from (
                    select unique cla.id cla_id
                         ...
                         , round(sum(fill/100*g.duration),grid_decimal_places) units --/count(unique cla.id)
                     from classes cla
                     , grids g
                     , gro_cla
                     , lec_cla
...
begin
  ...
  begin
    select VALUE into grid_decimal_places from system_parameters where name = 'GRID_DECIMAL_PLACES';
    exception
