  ...
  insert into tt_ids 
   ...
   from tt_combinations ttc 
    where --filter by lecturer. Ignore filter is there is no lecturer. Ignore lecturer is you are to select another lecturer
          (lec_id in (select Id from lecturers where id in (select Id from tmp_numbers)) OR (select Id from lecturers where id in (select Id from tmp_numbers)) is null OR p_rescat_id=g_lecturer ) 
     and  (sub_id in (select Id from subjects  where id in (select Id from tmp_numbers)) OR (select Id from subjects  where id in (select Id from tmp_numbers)) is null OR p_rescat_id=g_subject)
     and  (for_id in (select Id from forms     where id in (select Id from tmp_numbers)) OR (select Id from forms     where id in (select Id from tmp_numbers)) is null OR p_rescat_id=g_form)
     and  (gro_id in (select Id from groups    where id in (select Id from tmp_numbers)) OR (select Id from groups    where id in (select Id from tmp_numbers)) is null OR p_rescat_id=g_group)
     and  (per_id in (select Id from periods   where id in (select Id from tmp_numbers)) OR (select Id from periods   where id in (select Id from tmp_numbers)) is null OR p_rescat_id=g_period)
     and ttc.enabled = 'Y' 
