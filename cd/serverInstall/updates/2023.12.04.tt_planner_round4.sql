...
begin
  ...
  for rec in (
    select id, rowid, weight
     ...
     --
     update tt_combinations 
        set  avail_curr = avail_orig - round(l_fill_sum,4)
      where rowid = rec.rowid; 
  end loop;
...
begin
    ...
    for rec in (select unique per_id from tt_combinations where weight = 122 and per_id =nvl(pPER_ID, per_id) /*and integration_id is not null*/) loop
        ...
        merge into tt_combinations m 
            using (
                select lec_id, gro_id, nvl(str_elems.child_id, sub_id) sub_id, for_id, round(sum(fill/100*g.duration),4) units --/count(unique cla.id)
                 from classes cla
                 , grids g
                 , gro_cla
                 , lec_cla
            ...
            using (
                select * from (
                    select unique cla.id cla_id
                         ...
                         , round(sum(fill/100*g.duration),4) units --/count(unique cla.id)
                     from classes cla
                     , grids g
                     , gro_cla
                     , lec_cla
