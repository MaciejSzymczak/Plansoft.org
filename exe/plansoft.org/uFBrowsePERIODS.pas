...
begin
  ID := ROL_ID.Text;
  If LookupWindow(DModule.ADOConnection, 'PLANNERS','','NAME','NAZWA','NAME','(TYPE=''ROLE'' AND ID IN (SELECT ROL_ID FROM ROL_PLA WHERE PLA_ID = '+IntToStr(UserID)+'))','',ID) = mrOK Then
