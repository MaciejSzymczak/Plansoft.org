...
type
  ...
  private
    ...
    procedure SetCalColor(const Value: String);
  protected
  ...
  public
    ...
    property CalColor: String read FCalColor write SetCalColor;
  ...
  public
    ...
    function FindCalendar( calendarName : string) : TGCalendar;
...
begin
  ...
  if Calendars.CalendarCount > 0 then begin
   FTimeZone := Calendars.Calendars[0].TimeZone;
   FCalColor := Calendars.Calendars[0].CalColor;
  end
  else begin
   FTimeZone := 'Etc/GMT';
   FCalColor := '#2952A3';
        ...
        with ChildNodes[I] do
          if NodeName = 'gd:where' then
            ...
            if NodeName = 'gCal:timezone' then
              ...
              if NodeName = 'gCal:color' then
                Attributes['value'] := FCalColor;
    end;
    if Pos('<?xml version="1.0"?>', XMLDoc.XML.Text) <> 0 then
      Result := '<?xml version="1.0" encoding="UTF-8"?>' + UTF8Encode_(XMLDoc.DocumentElement.XML)
    else
...
procedure TGCalendar.SetCalColor(const Value: String);
begin
  if FCalColor <> Value then
  begin
    FCalColor := Value;
    FChanged := True;
  end;
...
function TGCalendars.FindCalendar(calendarName: string): TGCalendar;
var I : integer;
begin
  for I := 0 to self.CalendarCount - 1 do begin
    if self.Calendars[I].Name = calendarName then begin result :=  self[I]; exit; end;
  end;
  result := nil;
end;
