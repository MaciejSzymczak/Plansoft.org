...
begin
  ...
  if extractFileName(SysUtils.lowercase(Application.exeName)) <> 'planowanie.exe' then begin
   serror('Wykryto pr�b� omini�cia zabezpiecze� programu (1). Uruchomienie programu nie b�dzie mo�liwe do czasu, gdy nie zostan� przywr�cone poprzednie ustawienia. Application.exeName= ' + Application.exeName );
   dmodule.CloseDBConnection(true);
   Application.Terminate;
  end;

  if lastDateRun > UUtilityParent.DateToYYYYMMDD_HHMMSSMI(now)  then begin
    serror('Zmieniono dat� systemow� na tym komputerze, uruchomienie programu nie b�dzie mo�liwe do czasu, gdy nie zostan� przywr�cone poprzednie ustawienia');
    dmodule.CloseDBConnection(true);
    Application.Terminate;
  end else begin
    ...
    if getTerminalName <> extractWord(2,licenseType,[';']) then begin
     serror('Wykryto pr�b� omini�cia zabezpiecze� programu (2). Uruchomienie programu nie b�dzie mo�liwe do czasu, gdy nie zostan� przywr�cone poprzednie ustawienia. getTerminalName='+getTerminalName+' licenseType=' + licenseType);
     dmodule.CloseDBConnection(true);
     Application.Terminate;
    end;
    ...
    if licenseType =  'NOLIMIT' then begin
       ...
       if getTerminalName <> extractWord(2,installMarker,[';']) then begin
        serror('Wykryto pr�b� omini�cia zabezpiecze� programu (3). Uruchomienie programu nie b�dzie mo�liwe do czasu, gdy nie zostan� przywr�cone poprzednie ustawienia. getTerminalName='+getTerminalName+' installMarker='+installMarker);
        dmodule.CloseDBConnection(true);
        Application.Terminate;
       end;
      ...
      if daysRemaining > 0 then begin
        info('Uruchomiono demonstracyjn� wersj� programu Planowanie. Mo�esz korzysta� z tej wersji programu jeszcze przez nast�puj�c� liczb� dni: ' + intToStr(daysRemaining));
      end else begin
        warning('Uruchomiono demonstracyjn� wersj� programu Planowanie. Nie mo�esz ju� korzysta� z tej wersji programu, poniewa� min�o 120 dni od dnia, kiedy program zosta� zainstalowany. Skontaktuj si� z dostawc� oprogramowania');
        Application.Terminate;
      end;
    end;
 end;
