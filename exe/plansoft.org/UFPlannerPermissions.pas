...
Var x, y : Integer;
  ...
  Begin
   ...
   With DModule Do Begin
    ...
    OPENSQL('select * from '+Descriptor+'_pla where pla_id in ('+   'select id from planners where '+iif(editSharing,'0=0',' name='''+user+'''')+' and upper(name) like upper(''%'+getpSearch+'%'')'   +') and '+sqlWhereClause);
...
begin
  ...
  With DModule Do Begin
   OPENSQL('select * from planners where '+iif(editSharing,'0=0',' name='''+user+'''')+' and upper(name) like upper(''%'+getpSearch+'%'') ORDER BY TYPE DESC, NAME');
   ...
   OPENSQL('SELECT * FROM PLANNERS WHERE '+iif(editSharing,'0=0',' name='''+user+'''')+' and upper(name) like upper(''%'+getpSearch+'%'') and TYPE = ''USER''');
   ROLGrid.ColCount := QWork.RecordCount+1;
   ...
   if FProgramSettings.SQLLog.checked then writeLog (GetNowMarker + ' FplannerPermissions : PLANNERS');
   OPENSQL2('SELECT ID, '+sql_PLANAME+' NAME FROM PLANNERS WHERE '+iif(editSharing,'0=0',' name='''+user+'''')+' and TYPE in(''ROLE'',''EXTERNAL'') and upper(name) like upper(''%'+getRowSearch+'%'') ORDER BY '+sql_PLANAME);
  ...
  LoadCells('ROL', ROLGrid,'rol_id in (SELECT ID FROM PLANNERS  WHERE '+iif(editSharing,'0=0',' name='''+user+'''')+' and  TYPE = ''ROLE'' and upper(name) like upper(''%'+getRowSearch+'%''))');
