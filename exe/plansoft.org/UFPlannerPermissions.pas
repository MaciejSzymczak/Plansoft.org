...
Var x, y : Integer;
  ...
  Begin
   ...
   With DModule Do Begin
    ...
    OPENSQL('select * from '+Descriptor+'_pla where pla_id in ('+   'select id from planners where '+iif(isAdmin,'0=0',' name='''+user+'''')+' and upper(name) like upper(''%'+getpSearch+'%'')'   +') and '+sqlWhereClause);
...
begin
  ...
  With DModule Do Begin
   OPENSQL('select * from planners where '+iif(isAdmin,'0=0',' name='''+user+'''')+' and upper(name) like upper(''%'+getpSearch+'%'') ORDER BY TYPE DESC, NAME');
   ...
   OPENSQL('SELECT * FROM PLANNERS WHERE '+iif(isAdmin,'0=0',' name='''+user+'''')+' and upper(name) like upper(''%'+getpSearch+'%'') and TYPE = ''USER''');
   ROLGrid.ColCount := QWork.RecordCount+1;
   ...
   if FProgramSettings.SQLLog.checked then writeLog (GetNowMarker + ' FplannerPermissions : PLANNERS');
   OPENSQL2('SELECT ID, '+sql_PLANAME+' NAME FROM PLANNERS WHERE '+iif(isAdmin,'0=0',' name='''+user+'''')+' and TYPE in(''ROLE'',''EXTERNAL'') and upper(name) like upper(''%'+getRowSearch+'%'') ORDER BY '+sql_PLANAME);
  ...
  LoadCells('ROL', ROLGrid,'rol_id in (SELECT ID FROM PLANNERS  WHERE '+iif(isAdmin,'0=0',' name='''+user+'''')+' and  TYPE = ''ROLE'' and upper(name) like upper(''%'+getRowSearch+'%''))');
...
Procedure ShowModal;
Begin
 // can be launched by any user, but only planner can change everything
 //If not isAdmin Then Begin
 // Info('Ten modu³ mo¿e byæ uruchamiany tylko przez u¿ytkownika PLANNER');
 // Exit;
 //End;

 If Not Assigned(FPlannerPermissions) Then FPlannerPermissions := TFPlannerPermissions.Create(Application);
 FPlannerPermissions.btransfer.enabled := isAdmin;
