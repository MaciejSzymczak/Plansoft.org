...
begin
 result := GetCLASSESforL('CLASSES.ID', 'CLA_ID', nvl(CONL.Text, LSettings.Strings.Values['SQL.Category:DEFAULT']) ,ptablePostfix,LFilterType.text);
end;

function TFGenericFilter.sqlG ( ptablePostfix : string ) : string;
begin
 result := GetCLASSESforG('CLASSES.ID', 'CLA_ID', nvl(CONG.Text, GSettings.Strings.Values['SQL.Category:DEFAULT']) ,ptablePostfix,GFilterType.text);
end;

function TFGenericFilter.sqlR0 ( ptablePostfix : string ) : string;
begin
 result := GetCLASSESforR('CLASSES.ID', 'CLA_ID', nvl(conResCat0.Text, R0Settings.Strings.Values['SQL.Category:DEFAULT']) ,ptablePostfix,R0FilterType.text);
end;

function TFGenericFilter.sqlR1 ( ptablePostfix : string ) : string;
begin
 result := GetCLASSESforR('CLASSES.ID', 'CLA_ID', nvl(conResCat1.Text, R1Settings.Strings.Values['SQL.Category:DEFAULT']) ,ptablePostfix,R1FilterType.text);
end;
