...
Begin
 ...
 If LicznikZnakow = 0 Then ShowMessage('Wprowadzona maska nie niew≥aúciwa. Naleøy jπ zmieniÊ, poniewaø nie moøna autonumerowaÊ zleceÒ. Maska musi posiadaÊ znaki "09LlAaCc"');
...
Begin
 ...
 If WinControl is TDBDateEdit Then Begin
  Try
    ...
    End;
  Except
    If strIsEmpty(WinControl.Hint) Then addError(WinControl.Name+' musi zawieraÊ prawid≥owπ datÍ')
...
Begin
 ...
 If Name = 'SAVERESOURCES' Then
   If Trim(R.ReadString(Name))  = '' Then Begin
    ...
    //If Question('Program, na ktÛrym pracujesz zostanie odpowiednio skonfigurowany w zaleønoúci od mocy komputera.'+CR+
    //'Jeøeli wybierzesz TAK (lepszy komputer), program bÍdzie dzia≥a≥ nieco szybciej, ale bÍdzie pamiÍcioch≥onny.'+
    //'Jeøeli wybierzesz NIE (gorszy komputer), program bÍdzie dzia≥a≥ nieco wolniej, ale bÍdzie oszczÍdzia≥ pamiÍÊ.'+
    //'Jeøeli nie wiesz, jakπ odpowiedü wybraÊ, wybierz odpowiedü NIE') = idYES
...
Var Edit : TEdit;
Begin
 If Not Assigned(Application.MainForm) Then Begin SError('Aplikacja bez formularza g≥Ûwego, nie moøna wykonaÊ CopyToClipBoard S='+S); Exit; End;
...
initialization
 ...
 End;
  ...
  SError('Nie powid≥o siÍ usuniÍcie plikÛw. Sprawdü, czy moøesz usuwaÊ pliki z folderu:'+StringsPath+' Jeúli nie, to zg≥oú problem administratorowi');
 End;
