unit UFAdoLookupWindow;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  UFormConfig, StdCtrls, Grids, DBGrids, Buttons, ExtCtrls, Db, DBTables, UUtilityParent,
  ADODB;
...
type
  TFLookupWindow = class(TFormConfig)
    ...
    procedure BFILTERKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
    TableName, KeyField, DisplayFields, DisplayCaption, FindField, WhereClause, DefaultMask : String;
    MR : TModalResult;
...
Function LookupWindow(aADOConnection : TADOConnection; aTableName, aKeyField, aDisplayFields, aDisplayCaption, aFindField, aWhereClause, aDefaultMask : String; Var KeyValue : ShortString) : TModalResult;
...
begin
  ...
  SError('This module suportes only ORACLE, MSACCESS. This database is:'+DBType);

  ADOQuery.Close;
  Try
  ADOQuery.Open;
  Except
   SError('Blad przy:'+ADOQuery.SQL.Text);
   UUtilityParent.CopyToClipBoard(ADOQuery.SQL.Text);
   Raise;
  End;
  DBGrid.Columns[0].Width := 0;
  dbGrid.Columns[1].Width   := 1440;
  dbGrid.Columns[1].title.Caption   := DisplayCaption;
...
begin
  inherited;
 if (MR = mrOK) and (Empty(ADOQuery.Fields[0].AsString)) then CanClose := False;
end;
